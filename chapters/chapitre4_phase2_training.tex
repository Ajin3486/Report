...
\textbf{GPU parallelization for rapid training.} The choice of Isaac Lab is justified by its capability for massive parallelization on GPU. The number of simultaneous environments depends on the available GPU memory—higher-end GPUs enable more parallel simulations. In our setup using an NVIDIA RTX 4060, we can instantiate approximately 4000 environments in parallel, with each environment consuming roughly 1 environment per robot simulated. This allows collecting millions of experience samples in minutes, drastically accelerating training compared to real-time sequential simulation or standard CPU-based approaches. Each environment independently simulates the UR10 robot with randomized parameters, enabling robust policy learning across diverse conditions (see Figure \ref{fig:isaac_lab_training}).

\begin{figure}[!htb]
    \centering
    \includegraphics[width=1\textwidth]{images/train.png}
    \caption{Massive parallelization of thousands of environments in Isaac Lab.}
...
To prepare the policy for deployment on the physical UR10 robot and bridge the \textit{Sim2Real} gap (Reality Gap), real-world imperfections observed on our actual hardware setup are explicitly modeled in the simulator. 
...
\textbf{Camera model (Pinhole).} A virtual camera is placed with intrinsic parameters ($f_x=491.6, f_y=488.0$) directly extracted from the calibration of our physical Logitech C930e webcam (see Chapter 2). The camera's extrinsic pose (position and orientation relative to the robot base) is configured to match the exact mounting position on the real UR10 setup.
...
\subsubsection{Analysis of Training Dynamics}

To better understand the agent's behavior, we analyze the evolution of key training metrics (Figure \ref{fig:learning_curve}).
\begin{itemize}
    \item \textbf{Mean Episode Reward:} The primary metric starts near zero and rapidly plateaus around 375-380. This exponential growth demonstrates successful learning of tracking error minimization.
    \item \textbf{Mean Episode Length:} Quickly converges to the maximum horizon (64 steps), indicating that the agent learned to complete entire trajectories without early failures.
    \item \textbf{Policy Entropy:} Shows a gradual decrease, confirming the transition from exploration (high randomness) to exploitation (confident, deterministic strategy).
    \item \textbf{Value Function Loss:} Rapidly drops to near zero, proving the Critic network accurately predicts expected returns, essential for stable PPO updates.
\end{itemize}
