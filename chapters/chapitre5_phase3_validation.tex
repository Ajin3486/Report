...
We first evaluated the learned policy directly in the Isaac Lab training environment, but on scenarios never seen during training. The results show rapid and stable convergence.
...
Figure~\ref{fig:learning_curve} illustrates the training progress. The reward curve demonstrates stable convergence without instabilities, indicating efficient learning of the correction policy.
...
\section{Perspective: Sim-to-Sim Validation (Isaac \texorpdfstring{$\rightarrow$}{->} MuJoCo)}
To strengthen our validation, we exported the trained policy to a second simulator: MuJoCo.


\subsection{Transfer Methodology}
The transfer required converting the robot model:
\begin{itemize}
    \item Conversion from URDF format (used by ROS 2 and Isaac Lab) to MJCF format (specific to MuJoCo).
    \item Verification of frame correspondence and joint limits.
\end{itemize}

\subsection{Robustness Tests}
In MuJoCo, we subjected the agent to additional perturbations:
\begin{itemize}
    \item Addition of unknown masses on the end-effector (simulating a payload).
    \item Introduction of more severe calibration errors than during training.
\end{itemize}
