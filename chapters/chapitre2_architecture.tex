...
\begin{figure}[h]
\centering
\begin{overpic}[width=0.30\textwidth]{images/Robot_reel.jpg}
...
\textbf{Intrinsic parameters.} Camera intrinsics were calibrated using a checkerboard pattern, yielding the following calibration matrix $K$ and distortion coefficients $D$ (plumb\_bob model):
...
0 & 0 & 1 
\end{bmatrix}, \quad 
D = \begin{bmatrix} 0.149 \\ -0.675 \\ -0.002 \\ -0.001 \\ 1.382 \end{bmatrix}
\end{equation}
where $D=[k_1, k_2, p_1, p_2, k_3]^T$ represents the radial ($k_1, k_2, k_3$) and tangential ($p_1, p_2$) distortion coefficients used to rectify the raw camera images before marker detection.
